...
# Same shape as tests/test_agent.py's CONSENT_METADATA — profiles now require
# a valid consent record (see agent/profiler.py's consent gate, issue #8)
# before they can be created or loaded.
CONSENT_METADATA = {
...
class TestVariantAssignment:
    ...
    def test_assignment_persisted_on_profile_creation(self):
        ...
        profile = load_profile("brand_new_student", consent_metadata=CONSENT_METADATA)
    ...
    def test_preexisting_profile_without_variant_backfills_to_control(self):
        ...
        legacy_profile = {
            ...
            # no "experiment_variant" key, and no "consent" — simulates a
            # profile that predates both the experiment framework and the
            # consent gate, so it's quarantined until consent is supplied.
        }
        with open(_profile_path("legacy_student"), "w") as f:
            json.dump(legacy_profile, f)

        profile = load_profile("legacy_student", consent_metadata=CONSENT_METADATA)
...
class TestControlRegression:
    ...
    def test_control_bucketed_student_end_to_end_matches_pre_experiment_flow(self, monkeypatch):
        ...
        profile = profiler.record_attempt(
            "control_student", "cat", True, 5.0, ["CVC"], "animals", 1,
    ...
    def test_add_and_remove_variant_touches_only_the_registry(self):
        ...
        try:
            ...
            profiler.record_attempt(
                sid, "cat", True, 5.0, ["CVC"], "animals", 1,
                consent_metadata=CONSENT_METADATA,
            )
